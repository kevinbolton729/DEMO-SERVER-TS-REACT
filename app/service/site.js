 ...
 * @Last Modified time: 2018-04-24 09:10:30
 */
'use strict';
...
class SiteService extends Service {
  ...
  async EditSiteModel() {
    ...
    const fields = { _id: siteid || '59607e3c682e090ca074ecfd' }; // 查询字段集
    const updatefields = {
      // 更新字段
      siteName: name,
      siteInfo: info,
  ...
  async GetDocumentTitleModel() {
    ...
    // 查询字段集
    const fields = {
      _id: '59607e3c682e090ca074ecfd',
    };

    await Site.findOne(fields).exec((err, doc) => {
      if (err) {
        result.data = formatError();
      } else if (doc) {
        result.message = MSGETDATASUCCESS;
